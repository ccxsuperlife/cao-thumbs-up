...
public class SyncThumb2DBCompensatoryJob {
    ...
    public void run() {
        ...
        thumbKeys.stream().filter(ObjectUtil::isNotNull)
                .forEach(thumbKey -> needHandleDataSet.add(thumbKey.replace(ThumbConstant.TEMP_THUMB_KEY_PREFIX.formatted(""), "")));
        ...
        //补偿数据
        needHandleDataSet.forEach(timeSlice -> {
